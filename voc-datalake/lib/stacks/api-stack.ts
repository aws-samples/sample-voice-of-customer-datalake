...
export class VocApiStack extends VocStack {
  ...
  constructor(scope: Construct, id: string, props: VocApiStackProps) {
    ...
    this.api = new apigateway.RestApi(this, 'VocAnalyticsApi', {
      ...
      deployOptions: {
        ...
        methodOptions: {
          ...
          // complete set of verbs that cost anything.
    ...
    // 401s GENERATED BY THE GATEWAY (the MCP token authorizer refusing a
    // malformed Bearer shape) carry the RFC 6750 challenge from here — a
    // gateway response is the only place API Gateway lets us set this header
    // truthfully. 401s produced INSIDE a Lambda proxy integration are a
    // different story: REST API Gateway unconditionally remaps that header to
    // `x-amzn-remapped-www-authenticate` (documented, no opt-out — verified
    // live 2026-08-18), so mcp_handler keeps sending it and clients on this
    // endpoint receive it under the remapped name.
