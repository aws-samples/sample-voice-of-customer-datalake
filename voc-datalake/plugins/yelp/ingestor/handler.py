...
class YelpIngestor(BaseIngestor):
    ...
    def _parse_business_ids(self) -> list[str]:
        ...
        ids_str = self.secrets.get("business_ids", "")
        if not ids_str:
            return []
    ...
    def fetch_new_items(self) -> Generator[dict, None, None]:
        ...
        for business_id in self.business_ids:
            ...
            business_name = (
                business_info.get("name", business_id) if business_info else business_id
            )
            business_url = (
                business_info.get("url", f"https://www.yelp.com/biz/{business_id}")
                if business_info
                else f"https://www.yelp.com/biz/{business_id}"
            )
            ...
            metrics.add_metric(
                name=f"Yelp_{business_id}_Reviews", unit="Count", value=items_found
            )
