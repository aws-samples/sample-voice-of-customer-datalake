...
class TestPersonaToMarkdown:
    ...
    def test_full_persona(self):
        ...
        persona = {
            'name': 'Alice',
            'tagline': 'Power user',
            'quotes': [{'text': 'Love it'}, 'Simple quote'],
            'identity': {
                'age_range': '25-35', 'location': 'NYC', 'occupation': 'Dev',
                'income_bracket': 'High', 'education': 'BS', 'family_status': 'Single',
                'bio': 'A developer.'
            },
            'goals_motivations': {
                'primary_goal': 'Ship fast',
                'secondary_goals': ['Learn', 'Grow'],
                'success_definition': 'Happy users'
            },
            'pain_points': {
                'current_challenges': ['Slow builds'],
            ...
            'scenario': {
                ...
                'narrative': 'Alice opens her IDE...'
            }
        }
        md = _persona_to_markdown(persona)
        assert '# Alice' in md
        assert 'Power user' in md
        assert 'Love it' in md
        assert 'Simple quote' in md
        assert 'Demographics' in md
        assert 'Goals & Motivations' in md
    ...
    def test_minimal_persona(self):
        from projects import _persona_to_markdown
        md = _persona_to_markdown({'name': 'Bob'})
        assert '# Bob' in md
...
class TestDocumentToMarkdown:
    ...
    def test_content_with_heading(self):
        ...
        result = _document_to_markdown({'title': 'T', 'content': '# Already has heading\nBody'})
        assert result.startswith('# Already has heading')

    def test_content_without_heading(self):
        from projects import _document_to_markdown
        result = _document_to_markdown({'title': 'My Doc', 'content': 'Just text'})
        assert result == '# My Doc\n\nJust text'


class TestBuildSteeringFile:

    def test_full_steering(self):
        from projects import _build_steering_file
        project = {'name': 'Proj', 'description': 'Desc', 'kiro_export_prompt': 'Custom inst'}
        personas = [{'name': 'Alice', 'tagline': 'Tag'}]
        documents = [{'title': 'PRD', 'document_type': 'prd'}]
        result = _build_steering_file(project, personas, documents)
        assert 'Proj' in result
        assert 'Desc' in result
        assert 'Alice' in result
        assert 'PRD' in result
        assert 'Custom inst' in result
...
class TestAutoseedProject:
    ...
    @patch('projects.get_project')
    def test_full_autoseed(self, mock_get):
        from projects import autoseed_project
        mock_get.return_value = {
            'project': {'name': 'My Project', 'description': 'Desc'},
            'personas': [{'persona_id': 'p1', 'name': 'Alice', 'tagline': 'T'}],
            'documents': [{'document_id': 'd1', 'title': 'PRD Doc', 'content': 'Content', 'document_type': 'prd'}]
        }
        result = autoseed_project('proj1')
        assert result['project']['name'] == 'My Project'
        assert len(result['files']) == 3  # steering + 1 persona + 1 doc

    @patch('projects.get_project')
    def test_autoseed_with_filters(self, mock_get):
        from projects import autoseed_project
        mock_get.return_value = {
            'project': {'name': 'P'},
            'personas': [
                {'persona_id': 'p1', 'name': 'A'},
                {'persona_id': 'p2', 'name': 'B'},
            ],
            'documents': [
                {'document_id': 'd1', 'title': 'D1', 'content': 'C1', 'document_type': 'prd'},
                {'document_id': 'd2', 'title': 'D2', 'content': 'C2', 'document_type': 'research'},
            ]
        }
        result = autoseed_project('proj1', persona_ids=['p1'], document_ids=['d2'])
        # steering + 1 persona + 1 doc
