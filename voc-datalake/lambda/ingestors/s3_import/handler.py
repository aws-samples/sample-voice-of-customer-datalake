...
class S3ImportIngestor(BaseIngestor):
    ...
    def __init__(self, bucket: str = None, key: str = None):
        ...
        self.event_key = key
        self.import_bucket = bucket or self.secrets.get('s3_import_bucket', '')
    ...
    def _parse_csv(self, content: str, source_name: str) -> Generator[dict, None, None]:
        ...
        for row in reader:
            yield {
                ...
                'created_at': row.get('created_at', row.get('date', row.get('timestamp', ''))),
                'author': row.get('author', row.get('user', '')),
                'source_platform_override': source_name,
            }
    
    def _parse_json(self, content: str, source_name: str) -> Generator[dict, None, None]:
    ...
    def _normalize_json_item(self, item: dict, source_name: str) -> dict:
        ...
        return {
            ...
            'url': item.get('url', ''),
            'text': item.get('text', item.get('content', item.get('feedback', item.get('review', '')))),
            'rating': self._parse_rating(item.get('rating', item.get('score'))),
            'created_at': item.get('created_at', item.get('date', item.get('timestamp', ''))),
    ...
    def run_scheduled(self) -> dict:
        ...
        return {'status': 'success', 'files_processed': files_processed, 'items_processed': total_items}
