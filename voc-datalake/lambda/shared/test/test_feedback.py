...
class TestGetFeedbackContext:
    ...
    def test_queries_by_date_when_no_category_filter(self):
        ...
        mock_table.query.side_effect = [
            {'Items': [
                {'feedback_id': '1', 'source_platform': 'twitter'},
                {'feedback_id': '2', 'source_platform': 'trustpilot'}
            ]},
    ...
    def test_filters_by_source_platform(self):
        ...
        mock_table.query.side_effect = [
            {'Items': [
                {'feedback_id': '1', 'source_platform': 'twitter'},
                {'feedback_id': '2', 'source_platform': 'trustpilot'},
                {'feedback_id': '3', 'source_platform': 'twitter'}
            ]},
        ] + [{'Items': []} for _ in range(30)]
        
        result = get_feedback_context(
            mock_table,
            {'days': 7, 'sources': ['twitter']},
            limit=10
        )
    ...
    def test_combines_multiple_filters(self):
        ...
        mock_table.query.side_effect = [
            {'Items': [
                {'feedback_id': '1', 'source_platform': 'twitter', 'sentiment_label': 'positive', 'category': 'delivery'},
                {'feedback_id': '2', 'source_platform': 'trustpilot', 'sentiment_label': 'positive', 'category': 'delivery'},
                {'feedback_id': '3', 'source_platform': 'twitter', 'sentiment_label': 'negative', 'category': 'delivery'},
                {'feedback_id': '4', 'source_platform': 'twitter', 'sentiment_label': 'positive', 'category': 'support'},
            ]},
        ] + [{'Items': []} for _ in range(30)]
...
class TestFormatFeedbackForLlm:
    ...
    def test_handles_missing_optional_fields(self):
        ...
        assert 'Review 1' in result
        assert 'google_reviews' in result
    ...
    def test_formats_multiple_items(self):
        ...
        items = [
            {'source_platform': 'twitter', 'sentiment_label': 'positive', 'sentiment_score': 0.8, 'category': 'a', 'urgency': 'low', 'original_text': 'First'},
            {'source_platform': 'trustpilot', 'sentiment_label': 'negative', 'sentiment_score': -0.5, 'category': 'b', 'urgency': 'high', 'original_text': 'Second'},
        ]
...
class TestGetFeedbackStatistics:
    ...
    def test_calculates_source_counts(self):
        ...
        items = [
            {'sentiment_label': 'positive', 'source_platform': 'twitter'},
            {'sentiment_label': 'positive', 'source_platform': 'twitter'},
            {'sentiment_label': 'negative', 'source_platform': 'trustpilot'},
        ]
        
        result = get_feedback_statistics(items)
        
        assert 'twitter: 2' in result
        assert 'trustpilot: 1' in result
