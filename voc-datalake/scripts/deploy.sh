...
echo "   SECRET_ARN=\$(aws cloudformation describe-stacks --stack-name VocIngestionStack --query 'Stacks[0].Outputs[?OutputKey==\`ApiSecretsArn\`].OutputValue' --output text)"
echo "   aws secretsmanager put-secret-value --secret-id \"\$SECRET_ARN\" --secret-string '{...}'"
echo ""
echo "2. Test the API:"
