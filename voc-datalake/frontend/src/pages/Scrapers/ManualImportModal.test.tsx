...
describe('ManualImportModal', () => {
  ...
  describe('confirm flow', () => {
    ...
    it('prevents double-submit when import button is clicked twice rapidly', async () => {
      // Regression: the re-entrancy guard relies on isConfirming being read
      // synchronously after setState in the same async function. With useRef
      // this worked in-handler but not in UI; with useState we still need
      // disabling the button via UI state to prevent the second click.
      const user = userEvent.setup()
      let resolveConfirm: ((value: { success: boolean }) => void) | null = null
      mockConfirmManualImport.mockReturnValue(
        new Promise((resolve) => {
