...
export function ProblemStats({ categoryCount, subcategoryCount, problemCount, feedbackCount, urgentCount }: ProblemStatsProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4">
      <StatCard icon={<TrendingUp size={14} className="sm:w-4 sm:h-4" />} label="Categories" value={categoryCount} />
      <StatCard icon={<Layers size={14} className="sm:w-4 sm:h-4" />} label="Subcategories" value={subcategoryCount} />
      <StatCard icon={<AlertTriangle size={14} className="sm:w-4 sm:h-4" />} label="Problems" value={problemCount} />
      <StatCard icon={<MessageSquare size={14} className="sm:w-4 sm:h-4" />} label="Feedback" value={feedbackCount} />
