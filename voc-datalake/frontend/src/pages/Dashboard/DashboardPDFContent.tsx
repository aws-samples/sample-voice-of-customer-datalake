...
function getSentimentColor(name: string): string {
  if (name === 'positive') return '#22c55e'
  if (name === 'negative') return '#ef4444'
  if (name === 'mixed') return '#eab308'
  return '#6b7280'
}
...
function HeaderSection({ timeRange, totalFeedback, avgSentiment, urgentCount, sourcesCount }: DashboardPDFProps) {
  ...
  return (
    <div data-pdf-section style={{ marginBottom: '24px' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        {[
          { label: 'Total Feedback', value: String(totalFeedback), bg: '#f0f9ff', color: '#1d4ed8' },
          { label: 'Avg Sentiment', value: avgSentiment.toFixed(2), bg: '#f0fdf4', color: sentimentColor },
          { label: 'Urgent Issues', value: String(urgentCount), bg: '#fef2f2', color: '#991b1b' },
          { label: 'Sources Active', value: String(sourcesCount), bg: '#f5f3ff', color: '#5b21b6' },
        ].map(stat => (
          <div key={stat.label} style={{ padding: '12px 20px', backgroundColor: stat.bg, borderRadius: '8px', minWidth: '130px' }}>
...
function TrendSection({ dailyTotals }: { readonly dailyTotals: DailyTotal[] }) {
  ...
  return (
    <div data-pdf-section style={{ marginBottom: '28px' }}>
      <h2 style={{ fontSize: '18px', fontWeight: '600', color: '#1e293b', marginBottom: '12px' }}>📈 Feedback Volume Trend</h2>
        ...
        {sorted.map(day => {
          ...
          return (
            <div key={day.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
              <span style={{ fontSize: '9px', color: '#6b7280' }}>{day.count > 0 ? day.count : ''}</span>
              <div style={{
                width: '100%', maxWidth: '24px', height: `${height}px`,
                backgroundColor: '#3b82f6', borderRadius: '2px 2px 0 0',
              }} />
            </div>
          )
        })}
        ...
        {sentimentBreakdown.map(s => {
          ...
          return (
            <div key={s.name} data-pdf-section style={{
              padding: '12px 20px', borderRadius: '8px', border: '1px solid #e5e7eb', flex: '1', minWidth: '100px',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                <span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: getSentimentColor(s.name) }} />
                <span style={{ fontSize: '13px', fontWeight: '500', color: '#374151', textTransform: 'capitalize' }}>{s.name}</span>
              </div>
              <p style={{ fontSize: '22px', fontWeight: 'bold', color: '#1f2937', margin: '0 0 2px 0' }}>{s.value}</p>
              <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>{pct}%</p>
            </div>
          )
        })}
      </div>
...
function BreakdownTable({ title, emoji, entries, colorFn }: {
  readonly title: string; readonly emoji: string; readonly entries: BreakdownEntry[]
  ...
  return (
    <div data-pdf-section style={{ marginBottom: '28px', flex: '1', minWidth: '280px' }}>
      <h2 style={{ fontSize: '18px', fontWeight: '600', color: '#1e293b', marginBottom: '12px' }}>{emoji} {title}</h2>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
        <thead>
          <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
            <th style={{ textAlign: 'left', padding: '6px 8px', color: '#6b7280', fontWeight: '600' }}>Name</th>
            <th style={{ textAlign: 'right', padding: '6px 8px', color: '#6b7280', fontWeight: '600', width: '60px' }}>Count</th>
            <th style={{ textAlign: 'right', padding: '6px 8px', color: '#6b7280', fontWeight: '600', width: '60px' }}>Share</th>
            <th style={{ textAlign: 'left', padding: '6px 8px', color: '#6b7280', fontWeight: '600', width: '140px' }}></th>
          </tr>
        </thead>
        <tbody>
          {entries.map((entry, i) => {
            ...
            return (
              <tr key={entry.name} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                <td style={{ padding: '6px 8px', textTransform: 'capitalize', fontWeight: '500', color: '#1f2937' }}>
                  {entry.name.replace(/_/g, ' ')}
                </td>
                <td style={{ textAlign: 'right', padding: '6px 8px', color: '#374151' }}>{entry.value}</td>
                <td style={{ textAlign: 'right', padding: '6px 8px', color: '#374151' }}>{pct.toFixed(1)}%</td>
                <td style={{ padding: '6px 8px' }}>
                  <div style={{ width: '100%', height: '12px', backgroundColor: '#f3f4f6', borderRadius: '6px', overflow: 'hidden' }}>
                    <div style={{ width: `${pct}%`, height: '100%', backgroundColor: barColor, borderRadius: '6px', minWidth: pct > 0 ? '3px' : '0' }} />
                  </div>
                </td>
              </tr>
            )
...
function UrgentSection({ urgentItems }: { readonly urgentItems: readonly FeedbackItem[] }) {
  ...
  return (
    <div data-pdf-section style={{ marginBottom: '28px' }}>
      <h2 style={{ fontSize: '18px', fontWeight: '600', color: '#dc2626', marginBottom: '12px' }}>🚨 Urgent Issues</h2>
      {urgentItems.map(item => (
        <div key={item.feedback_id} data-pdf-section style={{
          padding: '10px 14px', borderLeft: '3px solid #ef4444', marginBottom: '8px',
          backgroundColor: '#fef2f2', borderRadius: '0 6px 6px 0',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <span style={{ fontSize: '12px', fontWeight: '500', color: '#374151', textTransform: 'capitalize' }}>
              {item.source_platform.replace(/_/g, ' ')} • {item.category.replace(/_/g, ' ')}
            </span>
            <span style={{ fontSize: '11px', color: '#6b7280' }}>
              {item.source_created_at ? new Date(item.source_created_at).toLocaleDateString() : ''}
            </span>
          </div>
          <p style={{ fontSize: '12px', color: '#374151', margin: '0 0 4px 0', lineHeight: '1.5' }}>
            {item.original_text.length > 200 ? item.original_text.slice(0, 200) + '…' : item.original_text}
          </p>
