      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
        ...
        <tbody>
          {categoryData.map((cat, i) => {
            ...
            return (
              <tr key={cat.name} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                <td style={{ padding: '8px 12px', textTransform: 'capitalize', fontWeight: '500', color: '#1f2937' }}>
                  <span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: cat.color, marginRight: '8px', verticalAlign: 'middle' }} />
                  {cat.name.replace(/_/g, ' ')}
                </td>
                <td style={{ textAlign: 'right', padding: '8px 12px', color: '#374151' }}>{cat.value}</td>
                <td style={{ textAlign: 'right', padding: '8px 12px', color: '#374151' }}>{percentage.toFixed(1)}%</td>
                <td style={{ padding: '8px 12px' }}>
                  <div style={{ width: '100%', height: '16px', backgroundColor: '#f3f4f6', borderRadius: '8px', overflow: 'hidden' }}>
                    <div style={{ width: `${percentage}%`, height: '100%', backgroundColor: cat.color, borderRadius: '8px', minWidth: percentage > 0 ? '4px' : '0' }} />
                  </div>
                </td>
              </tr>
            )
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
        {sentimentData.map(s => (
          <div key={s.name} data-pdf-section style={{
            ...
            <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>{s.percentage.toFixed(1)}%</p>
          </div>
        ))}
      </div>
    </div>
        ...
        {wordCloudData.map(item => {
          ...
          return (
            <span key={item.word} style={{
              padding: '4px 10px', backgroundColor: `rgba(59, 130, 246, ${intensity * 0.15})`,
              color: `rgba(30, 64, 175, ${0.5 + intensity * 0.5})`,
              borderRadius: '6px', fontSize: `${fontSize}px`, fontWeight: intensity > 0.6 ? '600' : '400',
            }}>
