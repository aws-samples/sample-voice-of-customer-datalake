...
import { Send, Bot, Loader2, Sparkles, PanelLeftClose, PanelLeft, Brain, X } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import { getDaysFromRange } from '../../api/client'
...
function ToolIndicator({ toolName }: Readonly<{ toolName: string }>) {
  return (
    <div className="flex items-center gap-2 text-xs text-purple-600 bg-purple-50 rounded-lg px-3 py-1.5 mb-2">
      ...
      <span>Searching: {toolName.replace(/_/g, ' ')}</span>
    </div>
  )
}
...
function StreamingMessage({
  ...
  thinking,
  activeTools,
}: Readonly<{ content: string; thinking: string; activeTools: string[] }>) {
  return (
...
export default function Chat() {
  ...
  const latestRef = useRef({ streamingText, thinkingText, streamError, sources, filters, activeConversationId, addMessage, t })
  useEffect(() => {
    latestRef.current = { streamingText, thinkingText, streamError, sources, filters, activeConversationId, addMessage, t }
  })

  // When streaming finishes, save the assistant message
  const prevStreamingRef = useRef(false)
  useEffect(() => {
    const { streamingText: text, thinkingText: thinking, streamError: error, sources: src, filters: f, activeConversationId: convId, addMessage: add, t: translate } = latestRef.current
    if (prevStreamingRef.current && !isStreaming && convId) {
      if (text) {
        add(convId, {
          role: 'assistant',
          content: text,
