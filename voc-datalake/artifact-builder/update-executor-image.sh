...
#
# This script:
# 1. Builds a new base image with updated code (AMD64 for ECS Fargate)
...
echo "=== Building new base image (AMD64) ==="
docker build --platform $PLATFORM -t artifact-builder-executor:latest executor/
...
docker run --rm --platform $PLATFORM -v /tmp/kiro-auth-transfer:/transfer artifact-builder-executor:with-auth \
...
CONTAINER_ID=$(docker run --platform $PLATFORM -d artifact-builder-executor:latest sleep 300)
...
# Commit the container as the new with-auth image (preserve the CMD)
echo "Committing new image..."
docker commit --change='CMD ["/bin/bash", "/app/entrypoint.sh"]' $CONTAINER_ID artifact-builder-executor:with-auth
...
echo ""
echo "=== Verifying auth ==="
docker run --rm --platform $PLATFORM artifact-builder-executor:with-auth bash -c "timeout 15 kiro-cli whoami"
